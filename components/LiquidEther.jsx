...
export default function LiquidEther({
  ...
  // { bass, mid, high } 0..1 written every RAF frame by the audio pipeline
  audioEnergyRef = null,
}) {
...
`;
    ...
    class WebGLManager {
      ...
      render() {
        const energy = this.props.audioEnergyRef?.current;
        if (energy) {
          const total = energy.bass + energy.mid * 0.6 + energy.high * 0.3;
          if (total > 0.01) {
            // visible but controlled drift
            const sim = this.output?.simulation;
            if (sim) sim.options.mouse_force = mouseForce + total * 18;
            this._audioAngle += 0.02 + energy.bass * 0.05;
            const r = 0.22 + energy.mid * 0.22;
            Mouse.setNormalized(
              Math.cos(this._audioAngle) * r,
              Math.sin(this._audioAngle * 0.7) * r
            );
            Mouse.isAutoActive = true;
            Mouse.autoIntensity = 0.7 + total * 1.8;
          } else {
            // restore base force when silent
            const sim = this.output?.simulation;
            if (sim) sim.options.mouse_force = mouseForce;
          }
        }
